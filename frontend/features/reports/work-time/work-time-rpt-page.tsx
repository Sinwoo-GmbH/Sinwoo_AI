...
import {
  ATTND_API_PATH,
  ATTND_RPT_API_PATH,
  type AttndCalDayResponse,
  type AttndWidgetResponse,
  type AttndWorkTimeFiltOptResponse,
  type AttndWorkTimeFiltOptsResponse,
  type AttndWorkTimeHistListResponse,
  type AttndWorkTimeHistRowResponse,
...
type MatrixRow = {
  ...
  empNm: string;
  deptNm: string;
  dayMap: Map<string, AttndWorkTimeHistRowResponse>;
};
...
function toHistQueryString(yearMonth: string, locale: LoginLocale) {
...
function generateMonthDays(yearMonth: string): AttndCalDayResponse[] {
...
function resolveHistKey(row: AttndWorkTimeHistRowResponse) {
  if (row.empNo) {
    return `emp:${row.empNo}`;
  }
  if (typeof row.empId === "number") {
    return `emp:EMP${String(row.empId).padStart(4, "0")}`;
  }
  if (typeof row.usrId === "number") {
    return `usr:USR${String(row.usrId).padStart(4, "0")}`;
  }
  return `name:${row.empNm}`;
}

function resolveOptKey(option: AttndWorkTimeFiltOptResponse) {
  return `emp:${option.refId}`;
}

function isSaturday(day: AttndCalDayResponse) {
  return new Date(`${day.dt}T00:00:00`).getDay() === 6;
}

function isSunday(day: AttndCalDayResponse) {
  return new Date(`${day.dt}T00:00:00`).getDay() === 0;
}

function resolveMatrixMarker(row: AttndWorkTimeHistRowResponse | undefined, day: AttndCalDayResponse) {
...
function resolveCellTitle(row: AttndWorkTimeHistRowResponse | undefined, day: AttndCalDayResponse) {
  const parts: string[] = [];
...
function rowMatchesKeyword(row: MatrixRow, keyword: string) {
  ...
  const empText = [row.empNo, row.empNm, row.deptNm].join(" ").toLowerCase();
  if (empText.includes(keyword)) return true;
...
export function WorkTimeRptPage({
  ...
  const [options, setOpts] = useState<AttndWorkTimeFiltOptsResponse | null>(null);
  const [history, setHist] = useState<AttndWorkTimeHistListResponse | null>(null);
  const [calDays, setCalDays] = useState<AttndCalDayResponse[]>([]);
  ...
  const fetchFiltOpts = useCallback(async () => {
    if (!accessToken) {
      onUnauthorized();
      return;
    }

    const response = await fetch(`${API_BASE_URL}${ATTND_RPT_API_PATH}/filter-options`, {
      headers: {
    ...
    const payload = (await response.json()) as AttndWorkTimeFiltOptsResponse;
    setOpts(payload);
  ...
  const fetchHist = useCallback(
    async (targetYearMonth: string) => {
      ...
      const response = await fetch(
        `${API_BASE_URL}${ATTND_RPT_API_PATH}/history?${toHistQueryString(targetYearMonth, locale)}`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
          },
      ...
      const payload = (await response.json()) as AttndWorkTimeHistListResponse;
      setHist(payload);
    },
  ...
  const fetchMonthMeta = useCallback(
    async (targetYearMonth: string) => {
      ...
      const response = await fetch(
        `${API_BASE_URL}${ATTND_API_PATH}/my/widget?yearMonth=${encodeURIComponent(targetYearMonth)}`,
        {
          headers: {
            Authorization: `Bearer ${accessToken}`,
      ...
      if (!response.ok) {
        setCalDays(generateMonthDays(targetYearMonth));
        return;
      }

      const payload = (await response.json()) as AttndWidgetResponse;
      setCalDays(payload.dayList.filter((item) => item.inMonthYn));
    },
    [accessToken, locale, onUnauthorized]
  );
  ...
  const runExport = useCallback(
    async (format: "excel" | "pdf") => {
      ...
      await runWithLoading(async () => {
        const response = await fetch(
          `${API_BASE_URL}${ATTND_RPT_API_PATH}/history/export/${format}?${toExportQueryString(yearMonth, keyword, locale)}`,
          {
            headers: {
              Authorization: `Bearer ${accessToken}`,
            },
  ...
  }, [fetchFiltOpts, runWithLoading]);

  useEffect(() => {
    void runWithLoading(async () => {
      await Promise.all([fetchHist(yearMonth), fetchMonthMeta(yearMonth)]);
    });
  }, [fetchHist, fetchMonthMeta, runWithLoading, yearMonth]);

  const visibleDays = useMemo(
    () => (calDays.length ? calDays : generateMonthDays(yearMonth)),
    [calDays, yearMonth]
  );
  ...
  const matrixRows = useMemo(() => {
    ...
    if (options?.empList?.length) {
      for (const option of options.empList) {
        const key = resolveOptKey(option);
        rowsByKey.set(key, {
          key,
          empNo: option.refCd ?? "",
          empNm: option.refNm ?? "",
    ...
    for (const entry of history?.itemList ?? []) {
      const key = resolveHistKey(entry);
      const current = rowsByKey.get(key) ?? {
        key,
        empNo: entry.empNo ?? "",
        empNm: entry.empNm ?? "",
        deptNm: entry.deptNm ?? "",
        dayMap: new Map<string, AttndWorkTimeHistRowResponse>(),
    ...
    >
      <div className="border-b border-slate-300 px-3 py-2">
        <div className="flex flex-col gap-2 xl:flex-row xl:items-start xl:justify-between">
          <div className="min-w-0">
            ...
            <h1
              id="wsp-work-time-history-title"
      ...
      <div className="border-b border-slate-300 bg-[#f3f5f8] px-3 py-2">
        <div className="flex flex-col gap-2 xl:flex-row xl:items-end">
          <div className="flex shrink-0 items-end gap-1">
            <Button
              id="wsp-work-time-history-export-excel"
              type="button"
              size="sm"
            ...
            <Button
              id="wsp-work-time-history-export-pdf"
              type="button"
              variant="outline"
              size="sm"
          ...
          <div className="grid flex-1 gap-3 sm:grid-cols-2 xl:grid-cols-[170px_minmax(0,1fr)]">
            <label className="space-y-1">
              ...
              <input
                id="wsp-work-time-history-month"
                type="month"
                value={yearMonth}
            ...
            <label className="space-y-1">
              ...
              <input
                id="wsp-work-time-history-keyword"
                type="text"
                value={keyword}
                placeholder={text.keywordPh}
      ...
      <div className="min-h-0 flex-1 p-2">
        <div className="flex h-full min-h-0 flex-col overflow-hidden rounded-[3px] border border-slate-300 bg-white">
          ...
          <div id="wsp-work-time-history-table-wrap" className="min-h-0 flex-1 overflow-auto">
            <table
              id="wsp-work-time-history-table"
