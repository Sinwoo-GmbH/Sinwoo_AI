...
export function BizModPage({
  ...
  const [bizMod, setBizMod] = useState<BizModResponse | null>(null);
  const [recs, setRecs] = useState<BizRecListResponse | null>(null);
  const [keyword, setKeyword] = useState("");
  const [yearMonth, setYearMonth] = useState("");
  const [selectedRow, setSelectedRow] = useState<BizRecRow | null>(null);
  const [draft, setDraft] = useState<BizRecRow | null>(null);
  const [related, setRelated] = useState<BizRelListResponse | null>(null);
  ...
  const loadRecs = useCallback(
    async (signal?: AbortSignal) => {
      ...
      const response = await fetch(`${API_BASE_URL}/api/v1/business/modules/${modCd}/records?${params.toString()}`, {
        method: "GET",
        headers: {
      ...
      const payload = (await response.json()) as BizRecListResponse;
      setRecs(payload);
      setSelectedRow((current) => reconcileSelectedRow(current, payload));
      setDraft((current) => reconcileSelectedRow(current, payload));
      setRecsFailed(false);
    },
    [accessToken, keyword, locale, modCd, onUnauthorized, yearMonth]
  );
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/v1/business/modules/${modCd}?${params.toString()}`, {
      method: "GET",
      headers: {
        Accept: "application/json",
        Authorization: `Bearer ${accessToken}`,
      ...
      .then(async (response) => {
        ...
        return response.json() as Promise<BizModResponse>;
      })
      .then((payload) => {
  ...
  }, [accessToken, locale, modCd, onLoadingChange, onUnauthorized]);

  useEffect(() => {
    const controller = new AbortController();
    setRecsFailed(false);
    loadRecs(controller.signal).catch(() => {
      if (!controller.signal.aborted) {
        setRecsFailed(true);
        setRecs(null);
      }
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/api/v1/business/modules/${modCd}/records/${selectedId}/related`, {
      method: "GET",
      headers: {
        Accept: "application/json",
        Authorization: `Bearer ${accessToken}`,
      ...
      .then(async (response) => {
        ...
        return response.json() as Promise<BizRelListResponse>;
      })
      .then((payload) => {
        if (controller.signal.aborted || !payload) return;
        setRelated(payload);
    ...
    return () => controller.abort();
  }, [accessToken, modCd, onUnauthorized, selectedId]);

  const tableCount = bizMod?.tblNms.length ?? 0;
  const tableRows = useMemo(
    () =>
      bizMod?.tblNms.map((tblNm, index) => ({
        tblNm,
        role: index === 0 ? copy.primaryTable : copy.relatedTables,
      })) ?? [],
    [copy.primaryTable, copy.relatedTables, bizMod?.tblNms]
  );
  const visibleCols = useMemo(
    () => recs?.colList.filter((col) => col.visibleYn).sort((a, b) => a.dspOrd - b.dspOrd) ?? [],
    [recs?.colList]
  );
  const editableCols = useMemo(
    () =>
      recs?.colList
        .filter((col) => col.writableYn && !col.keyYn)
        .sort((a, b) => a.dspOrd - b.dspOrd) ?? [],
    [recs?.colList]
  );
  const canEditSelection = Boolean(recs?.editableYn && selectedId != null);
  const canDeleteSelection = Boolean(recs?.deletableYn && selectedId != null);
  const canCreate = Boolean(recs?.creatableYn);
  ...
  const handleNew = () => {
    const emptyDraft = editableCols.reduce<BizRecRow>((acc, col) => {
      acc[col.colNm] = defaultDraftValue(col);
      return acc;
    }, {});
    setSelectedRow(null);
    setDraft(emptyDraft);
  ...
  const handleFieldChange = (col: BizRecColResponse, value: string | boolean) => {
    setDraft((current) => ({
      ...(current ?? selectedRow ?? {}),
      [col.colNm]: value,
    }));
  ...
  const handleSave = async () => {
    ...
    if (isUpdate && !recs?.editableYn) return;
    if (!isUpdate && !recs?.creatableYn) return;

    setSaving(true);
    try {
      const response = await fetch(
        `${API_BASE_URL}/api/v1/business/modules/${modCd}/records${isUpdate ? `/${selectedId}` : ""}`,
        {
      ...
      const payload = (await response.json()) as BizRecResponse;
  ...
  const handleDelete = async () => {
    if (!accessToken || selectedId == null || !recs?.deletableYn) return;
    setSaving(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/business/modules/${modCd}/records/${selectedId}`, {
        method: "DELETE",
        headers: {
          Accept: "application/json",
          Authorization: `Bearer ${accessToken}`,
  ...
  const handleStatus = async (aprvStsCd: string) => {
    ...
    setSaving(true);
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/business/modules/${modCd}/records/${selectedId}/status`, {
        method: "PATCH",
        headers: {
          Accept: "application/json",
          "Content-Type": "application/json",
  ...
  return (
    <div id={`business-mod-${bizMod.modCd.toLowerCase()}`} className="space-y-3">
      ...
      <section className="grid gap-3 md:grid-cols-4">
        <Metric icon={<Activity className="h-4 w-4" />} label={copy.itemCount} value={bizMod.itemCnt.toLocaleString()} />
        <Metric icon={<Database className="h-4 w-4" />} label={copy.primaryTable} value={bizMod.primaryTblNm} />
        <Metric icon={<GitBranch className="h-4 w-4" />} label={copy.relatedTables} value={tableCount.toString()} />
        <Metric icon={<ShieldCheck className="h-4 w-4" />} label={copy.moduleGroup} value={bizMod.groupCd} />
      </section>
      ...
      <section className="grid gap-3 xl:grid-cols-[1.1fr_0.9fr]">
        <div className="rounded-[4px] border border-slate-200 bg-white">
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              ...
              <tbody>
                {tableRows.map((row) => (
                  <tr key={row.tblNm} className="border-t border-slate-100">
                    <td className="px-3 py-2 font-mono text-xs text-slate-800">{row.tblNm}</td>
        ...
        <div className="rounded-[4px] border border-slate-200 bg-white">
          ...
          <div className="space-y-2 px-3 py-3 text-sm">
            <InfoRow label="Legacy code" value={bizMod.legacyMnuCd || copy.empty} />
            <InfoRow label="Legacy URI" value={bizMod.legacyUri || copy.empty} mono />
            <InfoRow label="TO-BE mnu" value={bizMod.mnuCd} mono />
            <InfoRow label="Module" value={bizMod.modCd} mono />
          </div>
        </div>
      </section>
          ...
          <div className="flex items-center gap-2 text-sm font-semibold text-slate-800">
            ...
            {recs ? (
              <span className="rounded-[4px] bg-slate-100 px-2 py-0.5 text-xs text-slate-600">
                {recs.totCnt.toLocaleString()}
              </span>
            ) : null}
          </div>
          <div className="flex items-center gap-2">
            {recs && !recs.editableYn ? (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-left text-sm">
              <thead className="bg-slate-50 text-xs uppercase text-slate-500">
                <tr>
                  {visibleCols.map((col) => (
                    <th key={col.colNm} className="whitespace-nowrap px-3 py-2 font-semibold">
                      {col.label}
                    </th>
                  ))}
                </tr>
              ...
              <tbody>
                {recs?.itemList.map((row, index) => {
                  ...
                  return (
                    ...
                    >
                      {visibleCols.map((col) => (
                        <td key={col.colNm} className="max-w-[220px] truncate px-3 py-2 text-slate-700">
                          {formatCell(row[col.colNm], col)}
                        </td>
                      ))}
                    </tr>
                  );
                })}
                {recs && recs.itemList.length === 0 ? (
                  <tr>
                    <td colSpan={Math.max(visibleCols.length, 1)} className="px-3 py-10 text-center text-sm text-slate-500">
                      {copy.empty}
                    </td>
                  </tr>
                ) : null}
            ...
            <div className="space-y-3 p-3">
              {editableCols.map((col) => (
                <Field
                  key={col.colNm}
                  col={col}
                  value={activeDraft?.[col.colNm] ?? ""}
                  disabled={saving || (!recs?.editableYn && selectedId != null)}
                  onChange={(value) => handleFieldChange(col, value)}
                />
              ))}
              ...
              {recs?.editableYn && selectedId != null && hasApprovalStatus(recs.colList) ? (
              ...
              {selectedId != null ? (
                <div className="space-y-2 border-t border-slate-200 pt-3">
                  ...
                  {related?.tableList.map((table) => (
                    <RelatedTable key={table.tblNm} table={table} />
                  ))}
...
function Field({
  ...
  const inputType =
    col.dataTpCd === "date"
      ...
      : col.dataTpCd === "time"
        ? "time"
        : col.dataTpCd === "datetime"
          ? "datetime-local"
          : col.dataTpCd === "number" || col.dataTpCd === "decimal"
            ? "number"
            : "text";

  return (
    <label className="block">
      <span className="mb-1 block text-xs font-semibold text-slate-600">{col.label}</span>
      <input
        type={inputType}
        step={col.dataTpCd === "decimal" ? "0.01" : undefined}
        value={inputType === "datetime-local" ? stringValue.slice(0, 16) : stringValue}
        disabled={disabled}
        required={col.requiredYn}
...
function RelatedTable({ table }: { table: BizRelListResponse["tableList"][number] }) {
  const previewCols = table.colList.slice(0, 4);
  return (
    <div className="rounded-[4px] border border-slate-200 bg-white">
          ...
          <table className="min-w-full text-left text-[11px]">
            ...
            <tbody>
              {table.itemList.slice(0, 3).map((row, index) => (
                <tr key={`${table.tblNm}-${index}`} className="border-t border-slate-100">
                  {previewCols.map((col) => (
                    <td key={col} className="max-w-[120px] truncate px-2 py-1 text-slate-600">
                      {formatCell(row[col], { dataTpCd: "text" } as BizRecColResponse)}
                    </td>
                  ))}
                </tr>
              ))}
...
function formatCell(value: string | number | boolean | null | undefined, col: BizRecColResponse) {
  if (value == null || value === "") return "-";
  if (col.dataTpCd === "decimal" || col.dataTpCd === "number") {
...
function defaultDraftValue(col: BizRecColResponse): string {
  if (col.dataTpCd === "yn") return "N";
  return "";
}

function getRowId(row: BizRecRow | null): number | null {
  if (!row) return null;
...
function reconcileSelectedRow(current: BizRecRow | null, payload: BizRecListResponse): BizRecRow | null {
...
function hasApprovalStatus(cols: BizRecColResponse[]) {
  return cols.some((col) => col.colNm === "APRV_STS_CD");
}
