...
type WorkspaceAttendanceCardProps = {
...
} satisfies Record<LoginLocale, Record<string, string | string[]>>;
...
function isLeaveDay(day: AttendanceCalendarDayRes) {
  return day.attndStsCd === "LEAVE";
}

function isBusinessTripDay(day: AttendanceCalendarDayRes) {
  return day.attndStsCd === "BUSINESS_TRIP";
}

export function WorkspaceAttendanceCard({
  accessToken,
  locale,
  onUnauthorized,
  onLoadingChange,
}: WorkspaceAttendanceCardProps) {
  ...
  const [manualDate, setManualDate] = useState(currentBusinessDate);
  ...
  useEffect(() => {
    void loadWidget(yearMonth);
  }, [loadWidget, yearMonth]);

  const saveManualEntry = useCallback(async (type: "check-in" | "check-out") => {
  ...
  }, [accessToken, loadWidget, locale, manualCheckIn, manualCheckOut, manualDate, onLoadingChange, onUnauthorized, yearMonth]);
    ...
    <Card id="workspace-rules-card" className="border-[#D7E2F5]">
      ...
      <CardContent className="space-y-1.5 pt-0">
        ...
        {loading ? (
          ...
          <div id="workspace-attendance-calendar" className="rounded-[14px] border border-slate-200/80 bg-white px-2 py-1.5">
            <div
                ...
                >
                  ...
                  <div className="mt-1 space-y-0.5">
                    ...
                    {isLeaveDay(day) ? (
                      ...
                      >
                        {messages.leaveChip as string}
                      </div>
                    ) : null}

                    {isBusinessTripDay(day) ? (
                      <div
                      ...
                      >
                        {messages.businessTripChip as string}
                      </div>
                    ) : null}

                    {!day.chkinTm && !isLeaveDay(day) && day.holidayYn && day.holidayNm ? (
