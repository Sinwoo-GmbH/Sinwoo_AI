...
const fallbackMenuPresentationMetadata: Record<string, MenuPresentationMetadata> = {
  ...
  MNU_CUSTOMER_EMPLOYEES: { icon: "users" },
...
const fallbackMenuTitleTranslations: Partial<Record<string, Partial<Record<LoginLocale, string>>>> = {
  ...
  MNU_CUSTOMER_EMPLOYEES: { en: "Employees", de: "Mitarbeiter", ko: "직원" },
  MNU_CUSTOMER_DEPARTMENTS: { en: "Departments", de: "Abteilungen", ko: "부서" },
  ...
  MNU_CUSTOMER_SUBSCRIPTION: { en: "Subscription", de: "Abonnement", ko: "구독" },
  ...
  MNU_BIZ_MST_EMP: { en: "Employees", de: "Mitarbeiter", ko: "직원" },
  MNU_BIZ_MST_DEPT: { en: "Departments", de: "Abteilungen", ko: "부서" },
...
export function getLocalizedWorkspaceFallbackViewModel(id: string, locale: LoginLocale): ViewModel {
  ...
  const resolvedId = fallbackWorkspaceViewModels[id] ? id : viewModelAlias[id] ?? "MNU_CUSTOMER_MY_TIME";
  const base = fallbackWorkspaceViewModels[resolvedId];
