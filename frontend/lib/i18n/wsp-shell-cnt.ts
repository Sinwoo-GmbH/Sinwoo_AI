...
export type WspTabCtxMnuCnt = {
  close: string;
  closeOther: string;
  closeAll: string;
};

const wspShellCnt: Record<LoginLocale, WspShellCnt> = {
  en: {
    ...
    quickActs: "Quick actions",
    ruleOne: "Admins can start in client mode and switch to admin mode when needed.",
    ruleTwo: "Mnus support four depths and are designed for admin-side configuration.",
    ...
    focusDesc: "Use these cards for queues, alerts, and upgrade prompts.",
    tableDesc: "Desktop web should feel like a real browser workspace, not a mobile card stack.",
    name: "Name",
    owner: "Owner",
    status: "Status",
    updated: "Updated",
  ...
  de: {
    ...
    focusDesc: "Diese Karten zeigen Warteschlangen, Warnungen und Upgrade-Auslöser.",
    tableDesc: "Desktop-Web soll wie ein ernsthafter Arbeitsbereich wirken, nicht wie ein mobiler Kartenstapel.",
    name: "Name",
  ...
  ko: {
    ...
    expanded: "펼침",
    focusTitle: "우선 처리 흐름",
    focusDesc: "큐, 경고, 업그레이드 유도 항목을 이 카드 영역에 배치합니다.",
    tableDesc: "데스크톱 웹은 모바일 카드 화면이 아니라 진짜 업무용 브라우저 워크스페이스처럼 보여야 합니다.",
    name: "이름",
    owner: "담당",
    status: "상태",
    updated: "갱신",
...
const wspTabCtxMnuCnt: Record<LoginLocale, WspTabCtxMnuCnt> = {
  en: {
    close: "Close",
...
export function getWspShellCnt(locale: LoginLocale): WspShellCnt {
  return wspShellCnt[locale];
}

export function getWspTabCtxMnuCnt(
  locale: LoginLocale
): WspTabCtxMnuCnt {
  return wspTabCtxMnuCnt[locale];
}
