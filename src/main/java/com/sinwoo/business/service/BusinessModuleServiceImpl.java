...
import com.sinwoo.business.dto.BusinessModuleMetricResponse;
...
public class BusinessModuleServiceImpl implements BusinessModuleService {
    ...
    private static final List<BusinessModuleDefinition> MODULES = List.of(
            module(
                    "REQ_WORK_TIME",
            ...
            module("REQ_EXPENSE", "MNU_BIZ_REQ_EXPENSE", "REQUEST", text("비용 신청", "Expense", "Ausgabe"),
            ...
            module("REQ_TRIP", "MNU_BIZ_REQ_TRIP", "REQUEST", text("출장 신청", "Business Trip", "Geschaeftsreise"),
            ...
            module("REQ_LEAVE", "MNU_CUSTOMER_LEAVE", "REQUEST", text("휴가 신청", "Vacation", "Urlaub"),
            ...
            module("CLAIM_EXPENSE", "MNU_BIZ_CLAIM_EXPENSE", "MY_CLAIMS", text("비용 청구", "Expense Claims", "Ausgaben"),
                    text("직원이 제출한 월 비용 청구 문서를 관리합니다.", "Manages monthly expense claim documents submitted by employees.", "Verwaltet monatliche Ausgabenabrechnungen."),
            ...
            module("CLAIM_TRAVEL_EXP", "MNU_BIZ_CLAIM_TRAVEL_EXP", "MY_CLAIMS", text("출장비 청구", "Travel Expense", "Reisekosten"),
            ...
            module("RPT_WORK_TIME", "MNU_BIZ_RPT_WORK_TIME", "REPORT", text("근무시간 리포트", "Work Time", "Arbeitszeit"),
                    text("직원/부서별 근무시간 이력을 조회합니다.", "Reports work-time history by employee and department.", "Berichtet Arbeitszeiten nach Mitarbeiter und Abteilung."),
                    "RPW", "/report/workTime", "TB_WORK_TIME_ENTRY", List.of("TB_WORK_TIME_ENTRY", "TB_ATTND"),
                    "SELECT COUNT(*) FROM TB_WORK_TIME_ENTRY WHERE " + ACTIVE_CO_SCOPE),
            module("RPT_MONTHLY_EXP", "MNU_BIZ_RPT_MONTHLY_EXP", "REPORT", text("월 비용 리포트", "Monthly Expenses", "Monatliche Ausgaben"),
            ...
            module("FIN_TXN_PURCHASE", "MNU_BIZ_FIN_TXN_PURCHASE", "FINANCE_TXN", text("매입", "Purchase", "Einkauf"),
            ...
            module("FIN_TXN_SALES", "MNU_BIZ_FIN_TXN_SALES", "FINANCE_TXN", text("매출", "Sales", "Verkauf"),
            ...
            module("FIN_TXN_OPEX", "MNU_BIZ_FIN_TXN_OPEX", "FINANCE_TXN", text("운영비", "OpEx", "Betriebsausgaben"),
            ...
            module("FIN_TXN_CORP_ACC", "MNU_BIZ_FIN_TXN_CORP_ACC", "FINANCE_TXN", text("법인계좌 거래", "Corporate Account", "Firmenkonto"),
            ...
            module("FIN_TXN_PAYROLL", "MNU_BIZ_FIN_TXN_PAYROLL", "FINANCE_TXN", text("급여", "Payroll", "Gehaltsabrechnung"),
            ...
            module("FIN_MGT_EXPENSE", "MNU_CUSTOMER_EXPENSE_REVIEW", "FINANCE_MGT", text("비용 관리", "Expense", "Ausgaben"),
                    ...
                    "SELECT COUNT(*) FROM TB_EXP_DOC WHERE " + ACTIVE_CO_SCOPE),
            module("FIN_MGT_BALANCE", "MNU_BIZ_FIN_MGT_BALANCE", "FINANCE_MGT", text("잔액 관리", "Balance", "Kontostand"),
                    ...
                    "SELECT COUNT(*) FROM TB_CORP_TXN WHERE " + ACTIVE_CO_SCOPE),
            module("FIN_MGT_CLOSING", "MNU_BIZ_FIN_MGT_CLOSING", "FINANCE_MGT", text("마감", "Closing", "Abschluss"),
            ...
            module("FIN_MGT_ASSET", "MNU_BIZ_FIN_MGT_ASSET", "FINANCE_MGT", text("자산", "Asset", "Anlage"),
            ...
            module("FIN_MGT_FIXED_OPEX", "MNU_BIZ_FIN_MGT_FIXED_OPEX", "FINANCE_MGT", text("고정 운영비", "Fixed OpEx", "Fixe Kosten"),
                    ...
                    "SELECT COUNT(*) FROM TB_OP_EXP WHERE " + ACTIVE_CO_SCOPE),
            module("FIN_MGT_FIXED_SALES", "MNU_BIZ_FIN_MGT_FIXED_SALES", "FINANCE_MGT", text("고정 매출", "Fixed Sales", "Fixumsatz"),
            ...
            module("FIN_RPT_LEDGER", "MNU_BIZ_FIN_RPT_LEDGER", "FINANCE_REPORT", text("계정 원장", "Account Ledger", "Sachkontenblatt"),
            ...
            module("FIN_RPT_ASSET", "MNU_BIZ_FIN_RPT_ASSET", "FINANCE_REPORT", text("자산 목록", "Asset List", "Anlagenliste"),
                    ...
                    "SELECT COUNT(*) FROM TB_ASSET WHERE " + ACTIVE_CO_SCOPE),
            module("FIN_RPT_ANNUAL", "MNU_BIZ_FIN_RPT_ANNUAL", "FINANCE_REPORT", text("연간 리포트", "Annual Reports", "Jahresberichte"),
            ...
            module("MST_EMP", "MNU_CUSTOMER_EMPLOYEES", "MASTER", text("직원", "Employees", "Mitarbeiter"),
                    text("직원 기본정보와 AS-IS HR 확장 프로필을 관리합니다.", "Manages employees and AS-IS HR profile extensions.", "Verwaltet Mitarbeiterprofile."),
                    "ADUE", "/admin/employeeMgt", "TB_EMP_HR_PROFILE", List.of("TB_EMP", "TB_EMP_HR_PROFILE"),
                    "SELECT COUNT(*) FROM TB_EMP_HR_PROFILE WHERE " + ACTIVE_CO_SCOPE),
            module("MST_DEPT", "MNU_CUSTOMER_DEPARTMENTS", "MASTER", text("부서", "Departments", "Abteilungen"),
                    text("부서와 휴가 기준을 관리합니다.", "Manages departments and leave defaults.", "Verwaltet Abteilungen."),
                    "ADUD", "/admin/deptMgt", "TB_DEPT", List.of("TB_DEPT"),
                    "SELECT COUNT(*) FROM TB_DEPT WHERE " + ACTIVE_CO_SCOPE),
            module("MST_ACC", "MNU_BIZ_MST_ACC", "MASTER", text("계정 코드", "Account Codes", "Kontencodes"),
            ...
            module("MST_CORP_ACC", "MNU_BIZ_MST_CORP_ACC", "MASTER", text("법인 계좌", "Corporate Accounts", "Firmenkonten"),
            ...
            module("MST_DAILY_ALLOWANCE", "MNU_BIZ_MST_DAILY_ALLOWANCE", "MASTER", text("일비", "Daily Allowance", "Tagegeld"),
            ...
            module("MST_CORP_CAL", "MNU_BIZ_MST_CORP_CAL", "MASTER", text("회사 달력", "Corporate Calendar", "Firmenkalender"),
            ...
            module("MST_ACC_MAPPING", "MNU_BIZ_MST_ACC_MAPPING", "MASTER", text("계정 매핑", "Account Mapping", "Kontenzuordnung"),
            ...
            module("MST_FI_STMT", "MNU_BIZ_MST_FI_STMT", "MASTER", text("재무제표", "Financial Statements", "Finanzberichte"),
    ...
    private static final Map<String, BusinessModuleDefinition> MODULE_BY_CODE = MODULES.stream()
            .collect(java.util.stream.Collectors.toUnmodifiableMap(BusinessModuleDefinition::moduleCd, item -> item));

    private final NamedParameterJdbcTemplate jdbcTemplate;

    @Override
    public BusinessModuleListResponse getBusinessModules(AuthenticatedUser authenticatedUser, Locale locale) {
        return new BusinessModuleListResponse(
                MODULES.stream()
                        .map(module -> toResponse(module, authenticatedUser, locale))
                        .toList()
        );
    }

    @Override
    public BusinessModuleResponse getBusinessModule(AuthenticatedUser authenticatedUser, String moduleCd, Locale locale) {
        BusinessModuleDefinition module = MODULE_BY_CODE.get(normalizeModuleCd(moduleCd));
        if (module == null) {
            throw new ApiException(HttpStatus.NOT_FOUND, "BUSINESS_MODULE_NOT_FOUND", "Business module not found");
        }
        return toResponse(module, authenticatedUser, locale);
    }

    private BusinessModuleResponse toResponse(
            BusinessModuleDefinition definition,
            AuthenticatedUser authenticatedUser,
            Locale locale
    ) {
        long itemCnt = queryCount(definition.countSql(), authenticatedUser);
        String tableSummary = String.join(", ", definition.tableNms());
        return new BusinessModuleResponse(
                definition.moduleCd(),
                definition.menuCd(),
                definition.groupCd(),
                definition.moduleNm().resolve(locale),
                definition.description().resolve(locale),
                definition.legacyMenuCd(),
                definition.legacyUri(),
                definition.primaryTableNm(),
                definition.tableNms(),
                itemCnt,
                List.of(
                        new BusinessModuleMetricResponse("ITEM_CNT", "Items", Long.toString(itemCnt)),
                        new BusinessModuleMetricResponse("PRIMARY_TABLE", "Primary table", definition.primaryTableNm()),
                        new BusinessModuleMetricResponse("TABLES", "Tables", tableSummary)
                )
        );
    }

    private long queryCount(String sql, AuthenticatedUser authenticatedUser) {
    ...
    private static String normalizeModuleCd(String value) {
        return value == null ? "" : value.trim().toUpperCase(Locale.ROOT);
    }

    private static BusinessModuleDefinition module(
            String moduleCd,
            String menuCd,
            String groupCd,
            LocalizedText moduleNm,
        ...
        return new BusinessModuleDefinition(
                moduleCd,
                menuCd,
                groupCd,
                moduleNm,
    ...
    private record BusinessModuleDefinition(
            String moduleCd,
            String menuCd,
            String groupCd,
            LocalizedText moduleNm,
